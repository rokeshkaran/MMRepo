@retry(2)
Feature: MoveTo
Check that files can be moved on MM Cloud

Background: Pre-condition
	Given User is logged into MindManager web app via Api
	Given User uploads following files to 'For QA Automation' folder of Mind Manager cloud via Api
		| ItemName            | ItemPath       |
...
@MindManager @MindManagerCloud @MoveFile @App_QA @App_Staging @App_Prod @Cleanup @Qase[WEB-2397] @Upd_Qase @Build_24.0.107
...
@MindManager @MindManagerCloud @MoveFile @App_QA @App_Staging @App_Prod @Cleanup @Qase[WEB-2395] @Upd_Qase @Build_24.0.107
Scenario: Verify_that_it_is_possible_to_move_map_to_a_folder
	...
	Then 'auto_test_SRND' item name is displayed on Content Manager footer
	Then 'file' icon is displayed on Content Manager footer
	When User waits for data loading
	When User clicks 'MoveFile' folder
	When User waits for data loading
	When User clicks button with 'Move To This Folder' name on Content Manager footer
	Then text 'auto_test_SRND.mmap @NewLinehas been moved to @NewLineMoveFile.' displayed in 'joint-flash-message' dialog
	Then Content Manager footer is not displayed
	Then 'auto_test_SRND' item is displayed on content manager page
	Then 'auto_test_SRND.mmap' mindmanager cloud item is saved in 'For QA Automation/MoveFile' folder from 'own' directory via API

@MindManager @MindManagerCloud @MoveFile @App_QA @App_Staging @App_Prod @Cleanup @Qase[WEB-2396] @Upd_Qase @Build_24.0.107
Scenario: Verify_that_it_is_possible_to_move_map_to_a_sub_folder
	When User clicks 'Move to' item in 'service-items-action' dropdown menu
	Then Content Manager footer is displayed on content manager page
	Then 'auto_test_SRND' item name is displayed on Content Manager footer
	When User waits for data loading
...
@MindManager @MindManagerCloud @MoveFile @App_QA @App_Staging @App_Prod @Cleanup @Qase[WEB-2526] @Upd_Qase @Build_24.0.107
...
@MindManager @MindManagerCloud @MoveFile @App_QA @App_Staging @App_Prod @Cleanup @Qase[WEB-2527] @Upd_Qase @Build_24.0.107
